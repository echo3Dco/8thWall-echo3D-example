...
export const placegroundScenePipelineModule = () => {
  ...
  const animateIn = (model, pointX, pointZ, yDegrees, newScale) => {
    const scale = Object.assign({}, startScale)

    model.rotation.set(0.0, yDegrees, 0.0)
    model.position.set(pointX, 0.0, pointZ)
    model.scale.set(scale.x, scale.y, scale.z)
    XR8.Threejs.xrScene().scene.add(model)
    
    let newEndScale = new THREE.Vector3(newScale, newScale, newScale)
    

    new TWEEN.Tween(scale)
      .to(newEndScale, animationMillis)
      .easing(TWEEN.Easing.Elastic.Out) // Use an easing function to make the animation smooth.
      .onUpdate(() => { model.scale.set(scale.x, scale.y, scale.z) })
  ...
  const placeObject = (pointX, pointZ) => {
		echo.findObject(function(data) {
		  echo.models.forEach(false, function(value, key) {
  		  animateIn(value.hologram, pointX, pointZ, Math.random() * 360, value.hologram.scale.x)     // loaded handler.
  		})
		})
